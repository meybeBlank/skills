...
**核心规则**：
1. **严格按照节点跳转规则执行**：必须按照每个节点定义的跳转规则跳转，回退后必须重新执行当前节点，跳转规则不变。
...
7. **编码隔离**：所有编码节点必须使用独立的子agent在隔离上下文中执行，禁止在主agent中直接编码。
8. **回退熔断**：任一"审查FAIL回退"或"测试失败回退"的循环，同一回退环连续回退达到 3 次仍未通过时，停止自动回退，向用户汇报卡点（失败原因、已尝试的修复、需用户决策的分歧），由用户决定继续、调整方案或终止。
...
9. **需求变更回退**：任何节点若发现需求本身需要调整（而非当前节点产出有误）——例如新暴露的场景、与用户确认后需求范围变化、下游发现上游需求存在根本性遗漏或矛盾——**必须先向用户确认**，确认后可回退到 `需求理解` 节点重新拆解。此类回退属于需求变更，**不计入任何回退环的熔断计数**。回退后从需求理解节点重新顺序流转，已有存档按存档规则新建追加，不覆盖。
10. **范式 Profile 驱动**：受项目结构影响的形态（契约产出形态、测试分层形态、审查探测方式、覆盖率工具落地）外置到 `.claude/nodes/profiles/<profile>.md`，按项目类型选定。
...
** 说明 **：本节点运行全量测试，测试命令从架构基线提取全部子项目测试命令，分别运行并合并统计结果。此节点禁止进行代码改动
** 下一个节点 **：
  - 全部通过（所有子项目）**且** 改动覆盖率达标 → `代码提交`
  - 存在失败用例 > 0 → 回退`验证矩阵`（连续失败回退达 3 次触发回退熔断，上报用户）
...
** 用户确认 **：**必须用户确认操作**
** 完整性核对 **：提交前核对验证矩阵中标记的场景是否前后端均已覆盖——"纯后端"行后端代码已实现、"纯前端"行前端代码已实现、"前后端组合"行两端均已实现且全量测试通过
