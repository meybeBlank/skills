...
E2E 测试须覆盖完整业务流程的主路径与关键异常路径，不可仅覆盖正向"快乐路径"。

- **每个业务流须覆盖正常完成路径**：从请求发起到数据库落库的完整正向链路的每个环节均须有断言覆盖
- **鉴权失败路径须覆盖**：未认证请求的 401、权限不足请求的 403 须有对应用例
- **业务异常回滚路径须覆盖**：输入校验失败、资源不存在、重复操作等业务异常场景须验证返回正确的错误码与错误信息
- **涉及并发/冲突的流程须覆盖**：资源竞争、重复提交等并发场景须验证数据一致性
- **编排层链路须有独立用例**：组合多个子系统的端到端业务流程须在各子系统 mock 边界分开后有独立的编排层用例
